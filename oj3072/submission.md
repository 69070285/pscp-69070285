...
```text
Create a program that takes text as input and outputs the count for each vowel (A, E, I, O, U).
```
...
```text
Step 1: Input the text and use ⁠lower()⁠ to convert all letters to lowercase for easier checking
Step 2: Use ⁠count()⁠ to get the count of each vowel
Step 3: If a vowel exists in the text, output that vowel along with its count
```
...
```text
This is the case that doesn’t have any vowel in the text.
```
...
```text
This is the case that has all the vowels.
```
...
```text
This is the case that has some vowels.
```
